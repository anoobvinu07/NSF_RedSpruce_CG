...
#########################################################
## budbreak of 2020 - budset of 2020  
#########################################################
...
# Population column creation
# bb_20_P_bs_20_P_blup$Population <- gsub("\\_.*", "", bb_20_P_bs_20_P_blup$Family)

# Correlation between the two
bb_20_P_bs_20_P_corr <- MCMCglmm(cbind(scale(BudBreak2020),scale(BudSet2020)) ~ trait - 1, 
                                 # fixed=Region,
                                 random=~us(trait):Population,
                                 rcov=~us(trait):units, # or idh
                                 family=c("gaussian","gaussian"),
...
# save the output
saveRDS(bb_20_P_bs_20_P_corr,"/home/Anoob/mydata/Anoob/MCMCglmm/plasticity_outputs/bb_20_P_bs_20_P_corr")

#########################################################
## budbreak of 2020 - budset of 2019  
#########################################################
...
# Population column creation
# bb_20_P_bs_19_P_blup$Population <- gsub("\\_.*", "", bb_20_P_bs_19_P_blup$Family)

# Correlation between the two
bb_20_P_bs_19_P_corr <- MCMCglmm(cbind(scale(BudBreak2020),scale(BudSet2019)) ~ trait - 1, 
                                 # fixed=Region,
                                 random=~us(trait):Population,
                                 rcov=~us(trait):units, # or idh
                                 family=c("gaussian","gaussian"),
...
# save the output
saveRDS(bb_20_P_bs_19_P_corr,"/home/Anoob/mydata/Anoob/MCMCglmm/plasticity_outputs/bb_20_P_bs_19_P_corr")


##########################################################
## Basic plasticity models
##########################################################
budset2019_plasticity_mod <- lmer(BudSet2019_Plasticity~Region+(1|Pop),na.action=na.omit,data=Plasticity) # omit the excess NAs with na.action
budset2020_plasticity_mod <- lmer(BudSet2020_Plasticity~Region+(1|Pop),na.action=na.omit,data=Plasticity)
budbreak2020_plasticity_mod <- lmer(BudBreak2020_Plasticity~Region+(1|Pop),na.action=na.omit,data=Plasticity)
growth2019_plasticity_mod <- lmer(growth2019_Plasticity~Region+(1|Pop),na.action=na.omit,data=Plasticity)
growth2020_plasticity_mod <- lmer(growth2020_Plasticity~Region+(1|Pop),na.action=na.omit,data=Plasticity)

sink("summary_lmer_plasticity_TD.txt")
summary(budset2019_plasticity_mod)
summary(budset2020_plasticity_mod)
summary(budbreak2020_plasticity_mod)
summary(growth2019_plasticity_mod)
summary(growth2020_plasticity_mod)
sink()

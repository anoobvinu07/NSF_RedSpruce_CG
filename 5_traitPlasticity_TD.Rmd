...
# # file for step 10
# file10_gw19 <- growthPlasticity_2019
# file10_gw19$Trait <- "Growth2019"
# file10_gw19 <- file10_gw19 %>% tidyr::pivot_longer(cols=c("home","away"),
#                                                            names_to = "Environment",
#                                                            values_to = "value")
...
heightGrowth_2019_modTD <- lmer(Growth ~ Garden * TD + Region + (1|Family) + (1|Population) + (1|mBed),
                            data=heightGrowth_2019, na.action = na.exclude)
summary(heightGrowth_2019_modTD)

# trait vs TD
growth2019_TD <-     ggplot(data=heightGrowth_2019,
...
heightGrowth_2020_modTD <- lmer(Growth ~ Garden * TD + Region + (1|Family) + (1|Population) + (1|mBed),
                            data=heightGrowth_2020, na.action = na.exclude)
summary(heightGrowth_2020_modTD)

# trait vs TD
growth2020_TD <-     ggplot(data=heightGrowth_2020,
                         aes(x= TD,y=Growth, color=Region)) + 
                          geom_point(aes(shape = Region), size=2, alpha = 0.1) + 
                            geom_smooth(method='lm', se=F, formula=y~poly(x,2)) +
                              scale_color_manual(values = c("Core" = "goldenrod2","Edge"="steelblue","Margin"="green2")) +
...
# # file for step 10
# file10_gw20 <- growthPlasticity_2020
# file10_gw20$Trait <- "Growth2020"
# file10_gw20 <- file10_gw20 %>% tidyr::pivot_longer(cols=c("home","away"),
#                                                            names_to = "Environment",
#                                                            values_to = "value")
...
# heightGrowth_2019_modTD <- lmer(Growth ~ Garden * TD + Region + (1|Family) + (1|Population) + (1|mBed), data=heightGrowth_2019, na.action = na.exclude)
summary(heightGrowth_2019_modTD)
```
...
# heightGrowth_2020_modTD <- lmer(Growth ~ Garden * TD + Region + (1|Family) + (1|Population) + (1|mBed), data=heightGrowth_2020, na.action = na.exclude)
summary(heightGrowth_2020_modTD)
```
